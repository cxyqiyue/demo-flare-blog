...
function getThemeStyles(): ThemeStyles {
  switch (__THEME_NAME__) {
    case "fuwari":
      return {
        header:
          "py-3 pr-4 text-left font-bold text-base tracking-tight fuwari-text-90",
        ...
        ruleText:
          "min-w-0 text-sm font-medium leading-relaxed fuwari-text-50 [&_p]:my-0 [&_ul]:my-1 [&_ol]:my-1 [&_li]:my-0.5 [&_a]:text-(--fuwari-primary) [&_a]:no-underline",
      };
    case "default":
    default:
      return {
        ...
        ruleText:
          "min-w-0 text-sm leading-relaxed text-muted-foreground [&_p]:my-0 [&_ul]:my-1 [&_ol]:my-1 [&_li]:my-0.5 [&_a]:no-underline [&_a:hover]:underline",
      };
  }
}

/**
 * 友链「本站信息 + 申请须知」区块，两栏并排布局，各自为独立表格。
 *
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      <table className="w-full table-fixed border-collapse">
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row.label}>
              <td className={`w-[45%] py-3 align-top ${styles.divider}`}>
                <span className={styles.label}>{row.label}</span>
              </td>
              <td className={`py-3 align-top ${styles.divider}`}>
                {row.markdown ? (
                  <div
