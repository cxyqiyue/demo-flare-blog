...
function UploadModalInternal({
  ...
  const folderSelector = onFolderChange ? (
    <FolderDropdown
    ...
    return createPortal(
      ...
      >
        ...
        >
          ...
          {/* Body */}
          <div className="px-6 space-y-5 overflow-y-auto custom-scrollbar flex-1 min-h-0 pb-2">
            {/* Folder Selector */}
            {folderSelector}
            ...
            <div className="flex flex-col gap-2">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 fuwari-btn-regular rounded-xl px-3 py-2.5 hover:border-(--fuwari-primary)/50 transition-colors text-sm fuwari-text-75"
              >
                <FolderInput
                  size={14}
                  strokeWidth={1.5}
                  className="fuwari-text-50 shrink-0"
                />
                {m.media_upload_btn_choose_folder()}
              </button>
              <p className="text-xs fuwari-text-50">
                {m.media_upload_folder_preserve_hint()}
              </p>
            </div>
            ...
            <div
              ...
              onDrop={onDrop}
              className={cn(
                "relative border border-dashed rounded-2xl py-10 px-6 flex flex-col items-center justify-center cursor-pointer transition-all duration-300",
                isDragging
                  ? "border-(--fuwari-primary) bg-(--fuwari-primary)/10"
                  : "border-(--fuwari-input-border) hover:border-(--fuwari-primary)/50 hover:bg-(--fuwari-primary)/5",
              )}
            >
              <div className="text-center space-y-2">
                <p className="text-sm sm:text-base font-medium fuwari-text-90">
                  {isDragging
                ...
                <p className="text-xs sm:text-sm fuwari-text-50">
                  {limitMb !== null
                    ? m.media_upload_support_any_file_limited({
            ...
            {queue.length > 0 && (
              <div className="space-y-4">
                <div className="flex items-center justify-between border-b border-(--fuwari-input-border) pb-2">
                ...
                <div className="space-y-2">
                  {queue.map((item) => (
                    <div
                      key={item.id}
                      className="rounded-xl border border-(--fuwari-input-border) bg-(--fuwari-input-bg) p-3 flex flex-col gap-2"
                    >
                      <div className="flex justify-between items-center text-xs sm:text-sm">
                        <span className="truncate max-w-40 fuwari-text-90 font-medium">
                          {item.name}
                        </span>
                        <span className="fuwari-text-50">{item.size}</span>
                      </div>

                      {/* Progress Bar */}
                      <div className="relative h-1.5 w-full rounded-full bg-(--fuwari-btn-regular-bg) overflow-hidden">
                        <div
                          className={cn(
                      ...
                      <div className="flex justify-between items-center text-xs sm:text-sm mt-1">
                        <div className="flex items-center gap-2">
                          {item.status === "UPLOADING" && (
                            <Loader2
                              size={12}
                              className="animate-spin text-(--fuwari-primary)"
                            />
                          )}
                          <span
                            className={cn(
          ...
          <div className="px-6 pb-6 pt-4 flex justify-end gap-2 shrink-0">
        ...
        <div className="px-6 space-y-6 overflow-y-auto custom-scrollbar flex-1 min-h-0 pb-2">
          ...
          <div className="flex flex-col gap-2">
            <button
              type="button"
              onClick={() => folderInputRef.current?.click()}
              className="w-full flex items-center justify-center gap-2 border border-border/30 bg-muted/5 px-3 py-2 hover:bg-muted/10 transition-colors text-xs font-mono uppercase tracking-widest text-muted-foreground hover:text-foreground"
            >
