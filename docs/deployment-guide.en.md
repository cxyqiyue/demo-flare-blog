...
| `DOMAIN` | Your blog domain (e.g., `blog.example.com`). Set to your worker's `xxx.workers.dev` for a pure workers.dev deployment |
...
| `ROUTE` | Set to `custom_domain` (or set variable `CUSTOM_DOMAIN=1`) to use the official custom_domain mode. Default is `routes` mode |
...
```jsonc
{
  ...
  // Default: zone-based routes.
  // Replace the active object below with
  // { "pattern": "blog.example.com", "custom_domain": true }
  // if you want the official custom_domain binding instead.
  // Use an empty array "routes": [] for a pure workers.dev deployment.
  "routes": [
...
Use the official custom_domain binding if your domain is a proxied custom domain in the Cloudflare dashboard:
...
- Default: `routes` mode, `pattern=${DOMAIN}/*`
- Set repository variable `ROUTE=custom_domain` (or `CUSTOM_DOMAIN=1`) to use the official custom_domain binding
- `DOMAIN` empty or ending with `.workers.dev`: the workflow emits `"routes": []` for a pure workers.dev deployment
